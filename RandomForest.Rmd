...
<p style="text-align:justify";>
En dessous de la fonction **_randomForest_**, nous avons les informations sur le type d'arbre (ici, arbre de classification), le nombre d'arbre (100), le « Out Of Bag estimate of error rate » (l'erreur Out Of Bag) qui s'élève ici à **`r round(mean(y2!=y)*100,3)`%** et enfin la matrice de confusion. L’erreur Out Of Bag est utilisée pour mesurer la performance des modèles d’agrégation. Il s’agit de l’erreur moyenne calculée, à chaque fois, sur les échantillons qui n’ont pas servis à calculer le modèle. Plus ce taux est faible, plus le modèle est juste.
</p>
...
<p style="text-align:justify";>
On voit que notre AUC s'élève ici à **`r auc_rf_roc`**.  Cela signifie donc que, face à deux individus, l'un frauduleux et l'autre étant sain, dans **`r auc_rf_pourcent`%** des cas, le modèle arrive à les distinguer.
</p>
