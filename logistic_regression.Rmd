...
<p style="text-align:justify";>
Enfin nous avons le nombre d'itérations de l'algorithme de Fisher qui est égale à 13. Cela signifie que l'algorithme de scoring de Fisher à eu besoin de **`r fisher`** itérations pour effectuer l'ajustement du modèle.
</p>
...
<p style="font-size:1em;
          text-align:justify">
Comme pour le SVM, la matrice de confusion va nous fournir des informations sur les erreurs de classification de la régression logistique.  Nous avons ici un taux d'erreur de classification de **`r erreur_log`%** et donc **`r exact_log`%** d'individus correctement classés. Le taux d'exatitude des individus correctement classés ici est inferieur à celui trouvé pour le SVM, mais cette différence n'est pas flagrante.
</p>
...
auc <- auc@y.values[[1]]
auc_log_roc<-round(as.numeric(auc),3)
...
<p style="font-size:1em;
          text-align:justify">
On rappelle que plus l'AUC tend vers 1 et plus la performance du modèle augmente. On voit que notre statistique est de **`r auc_log_roc`**. Cela signifie donc que, face à deux individus, l'un faisant défaut et l'autre étant sain, dans **`r auc_log_pourcent`%** des cas, le modèle arrive à distinguer la personne frauduleuse de la personne saine.
</p>
